...
- SST: $\sum (y_i - \bar{y})^2$, df = $n-1$
- SSE: $\sum (y_i - \hat{y}_i)^2$, df = $n-2$
- SSR: $\sum (\hat{y}_i - \bar{y})^2$, df = 1
...
### SSR
SSR = $\sum (\hat{y}_i - \bar{y})^2 = \hat{\beta}_1^2 \sum_1^n (x_i - \bar{x})^2 = S_{xy}^2 / S_{xx} = (\sum_1^n \frac{x_i - \bar{x}}{\sqrt{S_{xx}}} y_i)$
